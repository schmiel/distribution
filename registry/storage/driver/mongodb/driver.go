...
func New(url, databaseName string, config *connectionConfig) (*Driver, error) {
	...
	ctx, cancel := config.BuildContext()
	defer cancel()
...
func createGridFSDatabase(d *driver) error {
	countCtx, countCancel := d.BuildContext()
	defer countCancel()
	count, err := d.GridFSFilesCollection().CountDocuments(countCtx, bson.M{})
	if err != nil {
		return err
	}
	if count == 0 {
		...
		putCtx, putCancel := d.BuildContext()
		defer putCancel()
		err := d.PutContent(putCtx, tempFilename, tempContent)
		if err != nil {
			return err
		}
		deleteCtx, deleteCancel := d.BuildContext()
		defer deleteCancel()
		e := d.Delete(deleteCtx, tempFilename)
		if e != nil {
			return err
		}
	}
...
func enableDatabaseSharding(d *driver) error {
	...
	findCtx, findCancel := d.BuildContext()
	defer findCancel()
	singleResult := d.client.Database("config").Collection("databases").FindOne(findCtx, bson.M{"_id": d.db.Name()})
	if singleResult.Err() != nil {
	...
	if !configEntries.Partitioned {
		commandCtx, commandCancel := d.BuildContext()
		defer commandCancel()
		result := d.client.Database("admin").RunCommand(commandCtx, bson.M{"enableSharding": d.db.Name()})
		if result.Err() != nil {
			return result.Err()
		}
	}
...
func createShardKey(d *driver, collectionName string, shardKeyCmd interface{}) error {
	findCtx, findCancel := d.BuildContext()
	defer findCancel()
	cursor, err := d.client.Database("config").Collection("collections").Find(findCtx, bson.M{"_id": collectionName})
	if err != nil {
		return err
	}
	defer closeCursor(d, cursor)
	cursorCtx, cursorCancel := d.BuildContext()
	defer cursorCancel()
	if !cursor.Next(cursorCtx) {
		commandCtx, commandCancel := d.BuildContext()
		defer commandCancel()
		singleResult := d.client.Database("admin").RunCommand(commandCtx, shardKeyCmd)
		if singleResult.Err() != nil {
			return singleResult.Err()
		}
	}
...
func (d *driver) PutContent(ctx context.Context, p string, contents []byte) error {
	deleteErr := d.DeleteByFilename(p)
...
func dirStat(d *driver, path string) (storagedriver.FileInfo, error) {
	...
	cursorCtx, cursorCancel := d.BuildContext()
	defer cursorCancel()
	files, err = getAll(cursorCtx, cursor)
	if err != nil {
		return nil, err
	}
	if len(files) > 0 {
...
func closeCursor(d *driver, cursor *mongo.Cursor) {
	ctx, cancel := d.BuildContext()
	defer cancel()
...
func (d *driver) Move(ctx context.Context, sourcePath string, destPath string) error {
	...
	removeErr := d.DeleteByFilename(sourcePath)
	if removeErr != nil {
		return removeErr
	}
...
func (d *driver) DeleteByFilename(path string) error {
	file, err := d.GetFileByName(path)
	if err != nil {
		if err == mongo.ErrNoDocuments {
			return nil
...
func (d *driver) GetFileByName(path string) (*gridFsEntry, error) {
	ctx, cancel := d.BuildContext()
	defer cancel()
...
func (d *driver) BuildContext() (context.Context, context.CancelFunc) {
	return d.config.BuildContext()
}

func (c *connectionConfig) BuildContext() (context.Context, context.CancelFunc) {
	if c.timeout != nil {
